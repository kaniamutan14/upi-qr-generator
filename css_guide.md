...
1. **Is it a simple row OR column of items?** Use **flexbox** ΓÇö 1D, content-first, content distributes along a single axis.
2. **Does a nested element need to line up with its grandparent grid's tracks?** Use **subgrid** ΓÇö 2D, relationship-first, inherits parent tracks so grandchildren can align across siblings.
3. **Is it a complex page or component structure with rows AND columns?** Use **grid** ΓÇö 2D, layout-first, you define the skeleton and content fills it.
4. **Is the content a long flow of prose that should split into balanced columns?** Use **multi-column** ΓÇö 1D flow, newspaper-style.
5. **Are items of varied heights that need to be packed tightly?** Use **grid** with `grid-auto-flow: dense` today; reach for native masonry (aka "grid lanes") only when it ships in your Baseline target (see [┬º8](#8-grid-lanes-aka-masonry)).
6. **Does an element need to float above the page and stay spatially tethered to a trigger, even across DOM boundaries or stacking contexts?** Use **anchor positioning** ΓÇö `anchor-name` on the trigger, `position-anchor` on the overlay (see [┬º5](#5-native-overlays-anchor-positioning-and-stacking-contexts)).

### 1.2 Working principles

**Do:**

- Use logical properties (`inline-size`, `block-size`, `margin-inline`, `padding-block`, `inset-inline-start`) for layout dimensions and spacing ΓÇö see `css` (via `npx -y modern-web-guidance@latest retrieve "css"`) for full coverage.
...
- Reach for intrinsic sizing (`min-content`, `max-content`, `fit-content()`) and flexible tracks (`fr`, `minmax()`) before fixed `width`/`height` ΓÇö fewer media queries, more resilient layouts.
...
One-dimensional layout ΓÇö items flow along a single **main** axis with alignment on the **cross** axis. Reach for it for navbars, toolbars, item rows, and any single-row-or-column distribution.

**Do:**

- Establish a context with `display: flex` and set the main axis with `flex-direction` (`row` default).
- Use `flex-wrap: wrap` whenever overflow is a possibility ΓÇö `nowrap` without `overflow: auto/hidden` will spill on narrow viewports.
...
- Push a single item to the far end of the main axis with `margin-inline-start: auto` (or `margin-block-start: auto`) ΓÇö that's the standard escape hatch.
...
- Set `min-inline-size: 0` (or `min-width: 0`) on flex items that contain long unbreakable content (URLs, code, long strings) ΓÇö flex items won't shrink below their content size by default, causing overflow.

**Do not:**

- Don't reach for `justify-self` on flex items ΓÇö it only works on grid, block, and absolutely-positioned layouts. Use auto margins instead.
...
- Don't forget the axis flip: when `flex-direction: column`, `justify-content` aligns on the block axis and `align-items` aligns on the inline axis ΓÇö the opposite of the default.
- Don't size both the container and its children to fill each other ΓÇö that's a common source of overflow and surprising results. Give one side a definite size.
- Don't set both `flex-basis` and `width`/`inline-size` on the same item ΓÇö `flex-basis` takes precedence in a flex context and `width` is ignored. Use `flex-basis` (or the `flex` shorthand) as the single source of truth for sizing flex items.
...
Two-dimensional layout ΓÇö define rows AND columns explicitly, or let the engine derive them. Subgrid lets a nested grid inherit its parent's tracks so grandchildren align across siblings.

**Choosing grid features:**

- Do you know exactly how many columns you need?
  - **Yes** ΓÇö use explicit tracks (`grid-template-columns: 200px 1fr`, `repeat(3, 1fr)`, etc.)
    - Do different columns need different sizes (sidebar + main, header spanning all)? ΓåÆ use `grid-template-areas` for named, readable regions
    - Are all columns uniform or positioned purely by line number? ΓåÆ use `repeat(N, ...)` or named lines
  - **No** (responsive, unknown item count) ΓÇö use `repeat(auto-fit, minmax(min, 1fr))`
    - Should items on the last row stretch to fill remaining space? ΓåÆ `auto-fit`
    - Should empty last-row tracks hold their min size (preserving column ghost slots)? ΓåÆ `auto-fill`
- Do you need to place an item at a specific location?
  - **Yes** ΓÇö use `grid-column: <start> / <end>` or `grid-area: <name>`
  - **No** (just spanning multiple tracks, flow position doesn't matter) ΓÇö use `grid-column: span <n>`
- Do child elements need to inherit the parent grid's track sizes (ragged-edge alignment across siblings)?
  - **Yes** ΓÇö use subgrid on the affected axis
    - Is the number of children per cell variable? ΓåÆ subgrid **one axis only**; use `grid-auto-rows`/`grid-auto-columns` for the other
    - Is the child count fixed? ΓåÆ subgrid on both axes is fine
  - **No** ΓÇö standard grid, no subgrid needed

**Do:**

- Establish a context with `display: grid`.
- Use `grid-template-areas` for complex page-level layouts ΓÇö area names are self-documenting and the declaration can be aligned in rows and columns for at-a-glance readability.
...
- Use subgrid (`grid-template-columns: subgrid` or `grid-template-rows: subgrid`) to solve the "ragged edge" problem in card lists ΓÇö internal elements like titles, metadata, and CTAs line up across siblings.
...
- Don't expect `auto-fit`/`auto-fill` track size to come from item content ΓÇö it comes from the `repeat()` size argument.
...
- Don't use `repeat(auto-fit/auto-fill, ...)` without a definite `inline-size` on the container ΓÇö inside `display: inline-grid` or an unsized flex item, the container has no width to divide, making track counts unpredictable.
...
- Give the container a definite `block-size` whenever `container-type: size` is used ΓÇö without one, descendants collapse because size containment forces the container to ignore its content.

**Do not:**

- Don't use `block-size` as a `container-type` value ΓÇö it isn't valid. Use `size` for both axes.
...
- Use `popover` for transient, non-modal UI (flyouts, toasts, tooltips) ΓÇö lives in the top layer, no `z-index` management needed.
- Use `<dialog>` with `.showModal()` for modal interactions that require focus trapping and an inert backdrop.
- Don't combine `popover` and `.showModal()` on the same element ΓÇö they're mutually exclusive runtime states.
...
- Don't mix physical and logical keywords in a single `position-area` value ΓÇö pick one coordinate system.
...
- Use the `-webkit-line-clamp` + `display: -webkit-box` + `-webkit-box-orient: vertical` triad for multi-line truncation ΓÇö despite the prefix, this pattern is fully specified and not deprecated. Declare the unprefixed `line-clamp` shorthand alongside it; browsers that don't yet support it ignore the property harmlessly.
**Do not:**

- Don't use `overflow: scroll` when `auto` will do ΓÇö `scroll` forces scrollbars even when there's nothing to scroll.
- Don't reach for `overflow: hidden` when you only want to clip ΓÇö `hidden` establishes a scroll container that can be programmatically scrolled.
...
- Don't use `100vw` for full-width layout ΓÇö it ignores scrollbar width and causes horizontal overflow. Use `100%`, `100dvw`, or `100svw` instead.
...
- Treat `grid-template-rows: masonry` as a progressive enhancement only ΓÇö feature-detect with `@supports`.
